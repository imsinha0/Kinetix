...
def main():
    ...
    for r in runs:
        ...
        print(
            ...
            f"ev d1={f('eval/d1_success_depth1')} d2={f('eval/d2_success_depth1')} "
            f"| S_end_d1={f('boundary/S_end_d1')} S_start_d2={f('boundary/S_start_d2')} "
            f"D2={f('transfer/delta_2')} B21={f('transfer/B_2_1')}  {r.url}"
        )
        if args.history:
            h = r.history(keys=["timing/num_env_steps", "round", "eval/d1_success_depth1", "eval/d2_success_depth1"], pandas=False)
            rows = [x for x in h if x.get("eval/d1_success_depth1") is not None]
            step = max(1, len(rows) // 12)
            for x in rows[::step]:
                print(f"      {x['timing/num_env_steps']/1e6:6.1f}M r{x.get('round')} d1={x['eval/d1_success_depth1']:.3f} d2={x['eval/d2_success_depth1']:.3f}")
